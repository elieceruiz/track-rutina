import streamlit as st from datetime import datetime, timedelta from pymongo import MongoClient from zoneinfo import ZoneInfo
...
if not st.session_state.cronometro_activo: tipo = st.selectbox("Selecciona tipo de comida para iniciar cronómetro:", ["--", "Desayuno", "Almuerzo", "Cena", "Snack", "Break"])
...
if acostarse_op == "Sí": hora_acostarse = st.time_input("¿A qué hora te acostaste?", key="hora_acostarse")

if levantarse_op == "Sí": hora_levantarse = st.time_input("¿A qué hora te levantaste?", key="hora_levantarse")
...
if salida_op == "Sí": hora_salida = st.time_input("¿A qué hora saliste de casa?", key="salida")

if llegada_op == "Sí": hora_llegada = st.time_input("¿A qué hora llegaste al trabajo?", key="llegada")
